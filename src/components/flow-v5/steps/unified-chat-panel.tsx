...
export function UnifiedChatPanel({ phase, messages, isTyping, onSend, onAction, onSystemMsgClick, isVideoStage }: UnifiedChatPanelProps) {
  const [inputValue, setInputValue] = useState('')
  const bottomRef = useRef<HTMLDivElement>(null)
  const inputRef = useRef<HTMLInputElement>(null)
  const config = isVideoStage && phase === 'storyboard'
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Header */}
      <div className="shrink-0 border-b border-white/[0.08] glass-surface px-4 py-3">
                    ...
                    >
                      ...
                      <span className={cn(
                        ...
                        isPhaseMsg && onSystemMsgClick
                          ? 'text-amber-600 dark:text-amber-500/70 group-hover/phase:text-amber-500 dark:group-hover/phase:text-amber-400 group-hover/phase:underline underline-offset-2'
                          : 'text-amber-600 dark:text-amber-500/70',
                      )}>
                        {msg.text}
                      </span>
                      <div className="flex-1 border-t border-border" />
      ...
      <div className="border-t border-white/[0.08] px-4 py-3 space-y-2.5 shrink-0">
        <div className="flex flex-wrap gap-1.5">
          {suggestions.map((s) => (
            <button
              key={s}
              onClick={() => handleSend(s)}
              className="rounded-full border border-white/[0.08] glass-surface px-2.5 py-1 text-[10px] text-muted-foreground hover:bg-muted hover:text-foreground transition-colors cursor-pointer"
            >
              {s}
            </button>
          ))}
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 rounded-xl glass-input px-3.5 py-3 text-[12px] text-foreground placeholder:text-muted-foreground outline-none transition-all shadow-md shadow-black/10"
          />
          <button
            onClick={() => handleSend()}
