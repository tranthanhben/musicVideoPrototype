...
import { MV_TYPES } from '@/lib/flow-v5/mock-data'
import type { MvType } from '@/lib/flow-v5/types'
...
export function MvTypeStep({ selected, onSelect }: MvTypeStepProps) {
  return (
    <div
      className="relative flex h-full items-center justify-center p-6 overflow-hidden"
    >
      {/* Ambient blobs */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-24 -left-24 h-72 w-72 rounded-full opacity-20 dark:opacity-[0.07] blur-[100px]" style={{ background: 'radial-gradient(circle, #7C3AED, transparent 70%)', animation: 'blob-float-1 20s ease-in-out infinite' }} />
        <div className="absolute top-1/3 -right-16 h-56 w-56 rounded-full opacity-15 dark:opacity-[0.06] blur-[90px]" style={{ background: 'radial-gradient(circle, #EC4899, transparent 70%)', animation: 'blob-float-2 25s ease-in-out infinite' }} />
        <div className="absolute bottom-12 left-1/4 h-48 w-48 rounded-full opacity-15 dark:opacity-[0.05] blur-[80px]" style={{ background: 'radial-gradient(circle, #0EA5E9, transparent 70%)', animation: 'blob-float-3 22s ease-in-out infinite' }} />
      </div>
                ...
                >
                  ...
                  <div className="text-center mt-5 max-w-[11rem]">
                    ...
                    <p className={cn(
                      ...
                      isSelected ? 'text-foreground/60' : 'text-muted-foreground group-hover:text-foreground/60',
                    )}>
                      {type.description}
                    </p>
                  </div>
