...
import { FlowStepIndicator } from '@/components/flow/flow-step-indicator'
import { MvTypeStep } from '@/components/flow/steps/mv-type-step'
import { SetupStep } from '@/components/flow/steps/setup-step'
import { AnalysisStep } from '@/components/flow/steps/analysis-step'
import { StorylineStep } from '@/components/flow/steps/storyline-step'
import { MoodBoardStep } from '@/components/flow/steps/mood-board-step'
import { StoryboardStep } from '@/components/flow/steps/storyboard-step'
import { GenerationStep } from '@/components/flow/steps/generation-step'
import { VfxExportStep } from '@/components/flow/steps/vfx-export-step'
import { FLOW_STEPS, type FlowStep, type FlowConfig, type MvType, type RenderMode } from '@/lib/flow/types'
...
export default function FlowPage() {
  ...
  function canContinue(): boolean {
    switch (currentStep) {
      ...
      case 'storyline': return config.storylineId !== null
      case 'mood_board': return true
  ...
  function renderStep() {
    switch (currentStep) {
      ...
      case 'storyline':
        return (
          <StorylineStep
            selectedId={config.storylineId}
            onSelect={(id: string) => setConfig((c) => ({ ...c, storylineId: id }))}
            onContinue={nextStep}
          />
        )
      case 'mood_board':
        return <MoodBoardStep onContinue={nextStep} />
      case 'storyboard':
  ...
  const hasOwnContinue = ['analysis', 'storyline', 'mood_board', 'storyboard', 'generation', 'vfx_export'].includes(currentStep)
