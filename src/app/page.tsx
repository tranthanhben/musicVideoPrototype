...
import { SplitSquareVertical, Film, Workflow, LayoutGrid } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-16 bg-background">
      ...
      <motion.div className="mt-8 text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>
        <Link
          href="/flow-v3/showcase"
          className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-4 py-2 text-xs font-medium text-muted-foreground hover:text-foreground hover:border-primary/40 transition-colors"
        >
          <LayoutGrid className="h-3.5 w-3.5" />
          Product Components Showcase
          <span className="ml-1 transition-transform duration-200 hover:translate-x-0.5">→</span>
        </Link>
