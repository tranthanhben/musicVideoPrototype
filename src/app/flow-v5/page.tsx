...
import { FlowStepIndicator } from '@/components/flow-v5/flow-step-indicator'
import { MvTypeStep } from '@/components/flow-v5/steps/mv-type-step'
import { SetupStep } from '@/components/flow-v5/steps/setup-step'
import { UnifiedWorkspace } from '@/components/flow-v5/steps/unified-workspace'
import { FLOW_STEPS, type FlowStep, type FlowConfig, type MvType, type RenderMode } from '@/lib/flow-v5/types'
...
export default function FlowPage() {
  ...
  // Track workspace sub-phase for step indicator
  const [workspacePhase, setWorkspacePhase] = useState<string | null>(null)
  // Navigate to a sub-phase from the nav bar
  const [navigateToPhase, setNavigateToPhase] = useState<string | null>(null)
  const handleNavigateHandled = useCallback(() => setNavigateToPhase(null), [])
      ...
      case 'vfx_export':
        return (
          <UnifiedWorkspace
            ...
            onNavigateToPhaseHandled={handleNavigateHandled}
          />
        )
      default:
        return null
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden">
      {/* Step indicator */}
      <FlowStepIndicator currentStep={currentStep} onStepClick={goToStep} workspacePhase={isUnifiedStep(currentStep) ? workspacePhase : null} onSubPhaseClick={setNavigateToPhase} />
      ...
      {!isUnifiedStep(currentStep) && (
        <div className="shrink-0 border-t border-white/[0.08] glass-surface px-6 py-3 flex items-center justify-between">
          <button
            onClick={prevStep}
